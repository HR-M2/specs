...
- [x] 1.2.5 运行 `npm run build` 验证无 TypeScript 错误
- [x] 1.2.6 测试岗位管理页面功能
...
**步骤**:
- [x] 1.4.7 后端移除 `resume_data_id` 兼容代码
- [x] 1.4.8 更新 API 文档
...
**步骤**:
- [ ] 3.2.1 移除 `to_dict()` 中的 `"position": self.title` 行
- [ ] 3.2.2 运行后端验证
- [ ] 3.2.3 测试前端岗位页面正常
...
**步骤**:
- [ ] 3.3.1 对比 `ResumeData` 和 `Resume` 类型差异
- [ ] 3.3.2 将缺失字段合并到 `Resume`
- [ ] 3.3.3 全局替换 `ResumeData` → `Resume`
- [ ] 3.3.4 删除 `ResumeData` 类型定义
- [ ] 3.3.5 运行前端验证
